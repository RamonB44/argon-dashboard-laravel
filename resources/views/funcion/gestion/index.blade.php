...
@section('content')
    @include('layouts.navbars.auth.topnav', ['title' => 'Gestionar Funciones'])
    ...
    <div class="modal fade" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" id="createModal">
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                ...
                <form id="formcreate" action="{{ route('funcion.gestion.create') }}" method="post">
                    {{ csrf_field() }}
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true"></span></button>
                    <h4 class="modal-title">Editar funcion</h4>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            manageTable = $('#manageTable').DataTable({
                ...
                ajax: {
                    type: 'get',
                    url: '{{ route('funcion.gestion.getTable') }}',
                    beforeSend: function() {
                        $('.progress').show();
                    },
                    complete: function() {
        ...
        function eliminar(id) {
            if (confirm('Estas seguro de eliminar este registro?')) {
                // Deleted it!
                $.ajax({
                    url: url + '/funcion/gestion/delete/' + id,
                    type: 'GET',
        ...
        function edit(id) {
            $.ajax({
                url: url + '/funcion/gestion/update/' + id,
                type: 'GET',
                success: function(response) {
                    console.log(response);
