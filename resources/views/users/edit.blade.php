                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="offset-md-2  col-md-4">
                                ...
                                @endif
                                <input type="text" class="form-control" name="name" id="name" {{ $errors->has('name') ? 'is-invalid' : '' }} value="{{ $user->name }}" autofocus>
                            </div>
                            <div class="col-md-4">
                                <small>Rol</small>
                                <select name="group" id="group" class="form-control" >
                                    @foreach ($group as $item)
                                        <option <?php if($item->id == $user->user_group->first()->id){echo "selected";} ?> value="{{ $item->id }}">{{ $item->group_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="offset-md-2 col-md-4    ">
                                ...
                                @endif
                                <input type="email" name="email" id="email" class="form-control" {{ $errors->has('email') ? 'is-invalid' : '' }} value="{{ $user->email }}" autofocus>
                            </div>
                            <div class="col-md-4    ">
                                <small>Sedes</small>
                                @if ($errors->has('sedes'))
                                ...
                                <select name="sedes[]" id="sedes" class="form-control" multiple="multiple" >
                                    @foreach ($sedes as $item)
                                        <option <?php if(in_array($item->id,json_decode($user->user_group->first()->pivot->sedes))){ echo "selected";} ?> value="{{ $item->id }}">{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-11">
                                ...
                                <select name="areas[]" id="areas" class="form-control" multiple="multiple">
                                    @foreach ($areas as $item)
                                        <option <?php if(in_array($item->id,json_decode($user->user_group->first()->pivot->show_areas))){ echo "selected";} ?> selected value="{{ $item->id }}">{{ $item->area }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-11">
                                ...
                                <select name="treg[]" id="treg" class="form-control" multiple="multiple">
                                    @foreach ($treg as $item)
                                        <option <?php if(in_array($item->id,json_decode($user->user_group->first()->pivot->show_aux_treg))){ echo "selected";} ?> value="{{ $item->id }}">{{ $item->description }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
